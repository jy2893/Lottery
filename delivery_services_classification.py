...
region_dic = {} #初始化分拣后存放数据的字典地址
for item in pkg_list:
    province_name = item[1][:3] #取地址的前三个字符为省份
    if province_name.startswith("新疆"): #对特殊省份进行处理
        province_name = "新疆"
    if province_name.startswith("宁夏"):
        province_name = "宁夏"
    if not province_name in region_dic: #代表这是在字典中第一次出现的省的名称
        region_dic[province_name] = [item] #在字典里创造该省相对的的键
    else:
        region_dic[province_name].append(item) #在对应的值中插值

print('{')
for province_name in region_dic:
    print(f"\t'{province_name}':[")
    for value in region_dic[province_name]:
        print(f"\t\t{value},")
    print(f"\t],")
print('}')
